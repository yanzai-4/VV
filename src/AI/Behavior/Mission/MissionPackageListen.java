...
public class MissionPackageListen extends MissionPackage {
    ...
    // ##### 构造函数 #####
    // 创建新的任务
    public MissionPackageListen(ArrayList<String> input_origin, ActionLogicListen input_logic,
                                ArrayList<Integer> input_id) {
        super(input_origin, input_id, 3);
        comprehendList = new ArrayList<>();
        connectionList = new ArrayList<>();
        cognition = new ArrayList<>();
        type = MissionPackageType.Listen;
        query = null;
        logic = input_logic;
    }
    ...
    public boolean comprehend() {
        for (String temp_word : origin) {
            ...
            if (temp_cognition == null) {
                query = logic.incomprehension(temp_word);
                queryWord = temp_word;
                logic.addCognition(temp_word);
                return false;
            }
    ...
    public boolean thinking() {
        if (!comprehendList.isEmpty()) {
            ExperienceChain temp_chain = logic.getSolution(chainList, comprehendList, 75, "LISTEN");
            if (temp_chain != null) {
    ...
    public void moreThinking() {
        ExperienceChain temp_chain = logic.getSolution(chainList, comprehendList, 60, "LISTEN");
        if (temp_chain != null) {
            cognition = temp_chain.getSearchSituation();
            solution = temp_chain.getSolution();
        }
